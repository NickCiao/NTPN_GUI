...
class DataValidator:
    ...
    def validate_spike_data(data: list[np.ndarray]) -> None:
        ...
        if not isinstance(data, list):
            raise DataValidationError(f'Data must be a list, got {type(data).__name__}')
        ...
        for i, session_data in enumerate(data):
            if not isinstance(session_data, np.ndarray):
                raise DataValidationError(f'Session {i} must be numpy array, got {type(session_data).__name__}')
            ...
            if session_data.dtype not in [np.float32, np.float64, np.int32, np.int64]:
                warnings.warn(
                    f'Session {i} has unexpected dtype {session_data.dtype}, expected float32/float64/int32/int64'
                )

    @staticmethod
    def validate_labels(labels: list[np.ndarray], data: list[np.ndarray] | None = None) -> None:
        """
        ...
        if not isinstance(labels, list):
            raise DataValidationError(f'Labels must be a list, got {type(labels).__name__}')
        ...
        for i, session_labels in enumerate(labels):
            if not isinstance(session_labels, np.ndarray):
                raise DataValidationError(f'Label session {i} must be numpy array, got {type(session_labels).__name__}')

            if session_labels.ndim != 1:
                raise DataValidationError(f'Label session {i} must be 1D, got shape {session_labels.shape}')
...
def load_from_numpy(
    st_file: str | Path, context_file: str | Path, context_key: str
    ...
    try:
        ...
        with np.load(context_file, allow_pickle=True) as data:
            if context_key not in data:
                available_keys = list(data.keys())
                raise DataLoadError(
                    f"Key '{context_key}' not found in {context_file}. Available keys: {available_keys}"
                )
    ...
    try:
        ...
        with open(st_file, 'rb') as fp:
            ...
            if not isinstance(st_dict, dict):
                raise DataLoadError(f'Expected dict in {st_file}, got {type(st_dict).__name__}')
            if 'raw_stbins' not in st_dict:
                available_keys = list(st_dict.keys())
                raise DataLoadError(f"Key 'raw_stbins' not found in {st_file}. Available keys: {available_keys}")
        ...
        with open(context_file, 'rb') as fp:
            ...
            if not isinstance(label_dict, dict):
                raise DataLoadError(f'Expected dict in {context_file}, got {type(label_dict).__name__}')
...
def convert_to_safe_format(
    input_st_file: str | Path, input_context_file: str | Path, context_key: str, output_dir: str | Path | None = None
) -> tuple[Path, Path]:
    """
    ...
    np.savez(output_st_file, raw_stbins=np.array(spike_data_list, dtype=object))
    np.savez(output_context_file, **{context_key: np.array(labels_list, dtype=object)})
