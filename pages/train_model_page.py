...
from ntpn.model_service import compile_model, create_model
...
def define_model():
    ...
    define_model_trajectory_length = define_model_form.number_input(
        label='Trajectory Length', min_value=1, max_value=64, value=ntpn_constants.DEFAULT_TRAJECTORY_LENGTH
    )
    ...
    if define_model_submit:
        create_model(
            define_model_trajectory_length,
            define_model_number_classes,
            define_model_layer_width,
            define_model_trajectory_dimension,
            state=state,
        )
...
def train_model():
    ...
    with compile_tab:
        ...
        compile_learning_rate = compile_model_form.number_input(
            label='Learning Rate', min_value=0.001, max_value=0.9, value=ntpn_constants.DEFAULT_LEARNING_RATE
        )
        compile_model_submit = compile_model_form.form_submit_button(label='Compile Model')

        if compile_model_submit:
            compile_model(learning_rate=compile_learning_rate, state=state)
    ...
    with train_tab:
        ...
        train_model_epochs = train_model_form.number_input(
            label='Number of Epochs', min_value=1, max_value=100, value=ntpn_constants.DEFAULT_EPOCHS
        )
        train_model_submit = train_model_form.form_submit_button(label='Train Model')

        if train_model_submit:
            train_model(train_model_epochs, view=train_model_progress, state=state)
