...
def generate_sets():
    ...
    with cs_tab:
        ...
        if critical_sets_submit:
            ntpn_utils.generate_critical_sets(critical_sets_number_classes, critical_sets_number_samples, state=state)
...
def plotting():
    ...
    if plotting_selection == 'Critical Sets':
        ...
        if cs_plotting_submit:
            ntpn_utils.draw_cs_plots(cs_plotting_algo, cs_plotting_examples, cs_plotting_dims, cs_plotting_classes, state=state)
